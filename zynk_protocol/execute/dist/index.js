...
const createKeypairFromEnv = (envVar, fallbackLabel) => {
    ...
    if (privateKeyStr) {
        try {
            ...
            console.warn(`Generating a random keypair for ${fallbackLabel || envVar} instead`);
        }
    }
    else {
        console.warn(`No private key found for ${envVar}. Generating a random keypair for ${fallbackLabel || envVar} instead`);
    }
...
async function airdropSol(connection, address, amount) {
    try {
...
async function ensureAccountHasSOL(connection, address, minBalanceInLamports) {
    const balance = await connection.getBalance(address);
    if (balance < minBalanceInLamports) {
        console.log(`Current balance too low. Airdropping ${minBalanceInLamports / LAMPORTS_PER_SOL} SOL to ${address.toString()}`);
        await airdropSol(connection, address, minBalanceInLamports / LAMPORTS_PER_SOL);
...
async function sendTokens(program, connection, config, zynkOpWallet, tokenMint, amount, partnerOperationalWallet, partnerDepositWallet) {
    try {
        ...
        console.log("Partner Operational Wallet:", partnerOperationalWallet.toString());
...
async function main() {
    try {
        ...
        // Test send function
        try {
            const result = await sendTokens(program, connection, configAccount.publicKey, zynkOpWallet, tokenMint, 1000000, partnerOperationalWallet.publicKey, partnerDepositWallet.publicKey);
            ...
            const replenishResult1 = await replenishTokens(program, connection, configAccount.publicKey, result.orderTracker.publicKey, partnerDepositWallet, partnerDepositTokenAccount.address, paybackWallet.publicKey, 1, // orderId (should be 1 after initialization)
            ...
            const replenishResult2 = await replenishTokens(program, connection, configAccount.publicKey, result.orderTracker.publicKey, partnerDepositWallet, partnerDepositTokenAccount.address, paybackWallet.publicKey, 1, // orderId (should be 1 after initialization)
            ...
            const closeResult = await closeOrder(program, connection, configAccount.publicKey, result.orderTracker.publicKey, adminWallet, 1 // orderId (should be 1 after initialization)
            );
