...
pub(crate) fn create_order(
    ...
    // do we honour the supplied position_borrowed_amount.
    let is_orbit_call = ctx.accounts.orbit_authority.is_some();
    let effective_borrowed = if is_orbit_call {
        require!(position_borrowed_amount <= effective_amount, CoreError::InvalidOrder);
        position_borrowed_amount
    } else {
        // Non-Orbit callers must not set a position borrowed amount.
        require!(position_borrowed_amount == 0, CoreError::InvalidOrder);
        0
    };
